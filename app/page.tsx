...
<div style={{ padding: "0 48px", maxWidth: 1100, margin: "0 auto" }}>
  <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
    {[
      { name: "Mörk premium", tag: "Populär", tagBg: "#e0f5ee", tagColor: "#0d6e52", desc: "Professionell och auktoritativ. Passar investmentbolag, PE-firmor och IR-byråer som vill kommunicera styrka.", features: ["Mörk framsida med logga och nyckeltal", "Grön KPI-bar", "Inre sidor med diagram och tabeller"] },
      { name: "Ljus klassisk", desc: "Ren och lättläst. Passar de flesta bolag och rapporttyper. Enkel att anpassa för alla klienter.", features: ["Vit layout med grön accentlinje", "Fyra KPI-kort", "Stapeldiagram och resultaträkning"] },
      { name: "Djärv grön", desc: "Stark och igenkänningsbar. Bygger varumärke med varje rapport. Passar bolag som vill sticka ut.", features: ["Helgrön framsida", "VD-citat på inre sida", "Mörk KPI-sektion"] },
      { name: "Minimalistisk", desc: "Typografi och whitespace i fokus. Elegant och tidlös. Passar bolag som värdesätter enkelhet och precision.", features: ["Svart accentlinje överst", "Typografibaserad layout", "Tabellformat för nyckeltal"] },
      { name: "Årsredovisning", tag: "Nyhet", tagBg: "#e8f0fb", tagColor: "#2563a8", desc: "Komplett mall med VD-ord, förvaltningsberättelse, finansiella rapporter och noter.", features: ["Innehållsförteckning", "Förvaltningsberättelse", "ESEF/iXBRL-export inkluderat"] },
      { name: "IR-rapport", desc: "Investerarrelationer och kapitalmarknadskommunikation med aktiedata, nyckeltal och IR-kalender.", features: ["Aktiekurs och market cap", "IR-kalender med händelser", "Guidning och prognos"] },
    ].map((t, i) => (
      <div key={i} style={{ background: "#fff", border: "1px solid #dde8e2", borderRadius: 14, padding: "22px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ fontSize: 15, fontWeight: 500, color: "#0c1510", fontFamily: "sans-serif" }}>{t.name}</div>
          {t.tag && <span style={{ background: t.tagBg, color: t.tagColor, fontSize: 10, padding: "2px 8px", borderRadius: 20, fontWeight: 500 }}>{t.tag}</span>}
        </div>
        <div style={{ fontSize: 13, color: "#556b5e", lineHeight: 1.6, marginBottom: 16, fontFamily: "sans-serif", fontWeight: 300 }}>{t.desc}</div>
        <div style={{ height: 1, background: "#dde8e2", marginBottom: 14 }} />
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {t.features.map((f, j) => (
            <div key={j} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#2a3d32", fontFamily: "sans-serif" }}>
              <div style={{ width: 14, height: 14, borderRadius: "50%", background: "#e0f5ee", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <span style={{ color: "#0d6e52", fontSize: 8, fontWeight: 700 }}>✓</span>
              </div>
              {f}
            </div>
          ))}
